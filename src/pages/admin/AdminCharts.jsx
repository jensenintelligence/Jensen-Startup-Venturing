...
import { Image as ImageIcon, Sparkles, Trash2, Loader2, Upload, X } from "lucide-react";
  ...
  const analyze = async () => {
    ...
    try {
      ...
      try {
        ...
        await base44.entities.ChartAnalysis.update(ch.id, { ai_analysis: res.data.analysis, status: "completed" });
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div><h1 className="font-heading text-3xl font-semibold">Analisi Grafici</h1><p className="text-muted-foreground mt-1">Carica screenshot di grafici di trading e ottieni un'analisi tecnica AI.</p></div>
            ...
            <div key={c.id} className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${c.status === "completed" ? "bg-emerald-50 text-emerald-700" : c.status === "analyzing" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>{c.status === "completed" ? "Completata" : c.status === "analyzing" ? "In analisi" : "Fallita"}</span>
                </div>
                {(c.instrument || c.timeframe) && <p className="text-xs text-muted-foreground">{c.instrument} {c.timeframe ? `· ${c.timeframe}` : ""}</p>}
                {c.ai_analysis ? (
                  <div className="rounded-lg border border-accent/30 bg-accent/5 p-3 max-h-48 overflow-y-auto">
                    <p className="text-xs font-semibold text-accent inline-flex items-center gap-1.5 mb-1"><Sparkles className="h-3 w-3" /> Analisi AI</p>
                    <p className="text-xs text-foreground/80 whitespace-pre-wrap">{c.ai_analysis}</p>
                  </div>
